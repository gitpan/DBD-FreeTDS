...
# columns defined as  "float(6) null".  It really only tests 
...
$sql = "insert into $table values (1.0) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynullfloat6"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynullfloat6"} . " ";
        $value =~ s/ *$//;
    }
