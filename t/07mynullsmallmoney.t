...
# columns defined as  "smallmoney null".  It really only tests 
...
$sql = "insert into $table values (14.32) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynullsmallmoney"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynullsmallmoney"} . " ";
        $value =~ s/ *$//;
    }
    ...
    if (defined $value && $value ==  14.32)
    {
        if ($got_v1)
        {
            die "read 14.32 twice" ;
        }
        $got_v1 = 1;
    }
    elsif (! defined ($value))
