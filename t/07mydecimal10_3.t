...
# columns defined as  "decimal(10:3) not null".  It really only tests 
...
$sql = "insert into $table values (0.124) ";
...
$sql = "insert into $table values (-123.45) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mydecimal10_3"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mydecimal10_3"} . " ";
        $value =~ s/ *$//;
    }
    ...
    if (defined $value && $value ==  0.124)
    {
        if ($got_v1)
        {
            die "read 0.124 twice" ;
        }
        $got_v1 = 1;
    }
    elsif (defined $value && $value ==  -123.45)
    {
        if ($got_v2)
        {
            die "read -123.45 twice" ;
        }
        $got_v2 = 1;
    }
    else
