...
# columns defined as  "smallmoney not null".  It really only tests 
...
$sql = "insert into $table values (1.23) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mysmallmoney"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mysmallmoney"} . " ";
        $value =~ s/ *$//;
    }
    ...
    elsif (defined $value && $value ==  -1.23)
    {
        if ($got_v2)
        {
            die "read -1.23 twice" ;
