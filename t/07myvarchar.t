...
# columns defined as  "varchar(8) not null".  It really only tests 
...
$sql = "insert into $table values ('mickey') ";
...
$sql = "insert into $table values ('mouse') ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (defined $value && $value eq 'mickey')
    {
        if ($got_v1)
        {
            die "read 'mickey' twice" ;
        }
        $got_v1 = 1;
    }
    elsif (defined $value && $value eq 'mouse')
    {
        if ($got_v2)
        {
            die "read 'mouse' twice" ;
        }
        $got_v2 = 1;
    }
    else
