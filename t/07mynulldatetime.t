...
# columns defined as  "datetime null".  It really only tests 
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynulldatetime"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynulldatetime"} . " ";
        $value =~ s/ *$//;
    }
