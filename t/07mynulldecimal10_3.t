...
# columns defined as  "decimal(10:3) null".  It really only tests 
...
my $table = "t007mynulldecimal10_3";
...
$sql = "insert into $table values (123.345) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynulldecimal10_3"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynulldecimal10_3"} . " ";
        $value =~ s/ *$//;
    }
    ...
    if (defined $value && $value ==  123.345)
    {
        if ($got_v1)
        {
            die "read 123.345 twice" ;
        }
        $got_v1 = 1;
    }
    elsif (! defined ($value))
