...
# columns defined as  "smallint null".  It really only tests 
...
$sql = "insert into $table values (-126) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynullsmallint"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynullsmallint"} . " ";
        $value =~ s/ *$//;
    }
