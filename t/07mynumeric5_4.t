...
# columns defined as  "numeric (5:4) not null".  It really only tests 
...
$sql = "insert into $table values (12345.1234) ";
...
$sql = "insert into $table values (-5.0) ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynumeric5_4"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynumeric5_4"} . " ";
        $value =~ s/ *$//;
    }
    ...
    if (defined $value && $value ==  12345.1234)
    {
        if ($got_v1)
        {
            die "read 12345.1234 twice" ;
        }
        $got_v1 = 1;
    }
    elsif (defined $value && $value ==  -5.0)
    {
        if ($got_v2)
        {
            die "read -5.0 twice" ;
