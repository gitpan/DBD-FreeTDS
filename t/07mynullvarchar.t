...
# columns defined as  "varchar(40) null".  It really only tests 
...
$sql = "insert into $table values ('This is a string') ";
...
while (defined ($rs = $sth->fetchrow_hashref))
{
    ...
    if (! defined($r{"mynullvarchar"}))
    {
        undef $value;
    }
    else
    {
        $value = $r{"mynullvarchar"} . " ";
        $value =~ s/ *$//;
    }
    ...
    if (defined $value && $value eq 'This is a string')
    {
        if ($got_v1)
        {
            die "read 'This is a string' twice" ;
        }
        $got_v1 = 1;
    }
    elsif (! defined ($value))
